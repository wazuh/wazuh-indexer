...
```console
docker exec -it wi-build_$(jq -r .version<VERSION) bash packaging_scripts/build.sh -a {x64|arm64} -d {rpm|deb|tar}
```
...
```console
docker exec -it wi-assemble_$(jq -r .version<VERSION) bash packaging_scripts/assemble.sh -a x64 -d deb
```
...
```console
docker exec -it wi-assemble_$(jq -r .version<VERSION) bash packaging_scripts/assemble.sh -a x64 -d rpm
```
