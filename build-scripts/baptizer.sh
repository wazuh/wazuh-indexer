...
function usage() {
    ...
    echo -e "-t ALERTING_HASH\t[Optional] Commit hash from the wazuh-indexer-alerting repository"
...
function parse_args() {
    ...
    while getopts ":hp:a:d:r:l:e:s:n:t:mx" arg; do
        case $arg in
        h)
            usage
            exit 1
...
function get_devel_name() {
    ...
    if [ -n "$PLUGINS_HASH" ] && [ -n "$REPORTING_HASH" ] && [ -n "$SECURITY_HASH" ] && [ -n "$NOTIFICATIONS_HASH" ] && [ -n "$ALERTING_HASH" ]; then
        COMMIT_HASH="$GIT_COMMIT"-"$PLUGINS_HASH"-"$REPORTING_HASH"-"$SECURITY_HASH"-"$NOTIFICATIONS_HASH"-"$ALERTING_HASH"
    fi
