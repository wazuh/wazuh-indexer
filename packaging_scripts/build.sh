...
function usage() {
    ...
    echo -e "-b BRANCH\t[Optional] Branch from wazuh/wazuh to download the index template from, default is $(jq -r .version < VERSION.json)"
...
function parse_args() {
    ...
    [ -z "$BRANCH" ] && BRANCH="$(jq -r .version < VERSION.json)"
