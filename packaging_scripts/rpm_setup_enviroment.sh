...
if [ "$1" = "x64" ]; then
   ...
   python3 modules/allocation/main.py --action create --provider aws --size large --composite-name  nulix-centos-8-arm64 --instance-name "centos_8_arm_medium_vagrant" --inventory-output "/tmp/inventory.yaml" --track-output "/tmp/track.yaml" --label-team indexer --label-termination-date 1d --working-dir /tmp/indexer
else
