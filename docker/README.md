...
```console
docker build --build-arg="VERSION=4.10.5" --build-arg="INDEXER_TAR_NAME=wazuh-indexer-4.10.5-1_linux-x64_cfca84f.tar.gz" --tag=wazuh-indexer:4.10.5 --progress=plain --no-cache .
```

Then, start a container with:

```console
docker run -it --rm wazuh-indexer:4.10.5 
```
